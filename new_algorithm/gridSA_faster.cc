...
vector<int> simulatedAnnealing(const vector<Point>& P, const vector<int>& seed, int thread_id = 0) {
    ...
    // 스레드별로 다른 seed 사용 (충돌 방지)
    std::mt19937 rng(1234 + thread_id);  // 1234는 base seed
    std::uniform_real_distribution<double> U(0.0, 1.0);
    std::uniform_int_distribution<int> pick_idx(1, seed.size() - 2); // 경로 인덱스 선택
...
TSPResult gridSA_fast(const vector<Point>& P)
{
    ...
    for (int g = 0; g < G; ++g) {
        ...
        // omp_get_thread_num()을 사용해서 thread_id 전달
        int thread_id = omp_get_thread_num();
        sub[g] = simulatedAnnealing(P, seed, thread_id);
    }
